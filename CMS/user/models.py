...
from CMS.utils import BaseModel, ULIDField, UserAccountManager
...
class PhoneNumber(BaseModel):
    ...
    phone = models.CharField(
        _("phone number"), blank=True, help_text="phone number"
    )
...
class Role(BaseModel):
    ...
    slug = models.SlugField(
        _("role slug"), max_length=100, unique=True, db_index=True
    )
...
class CompanyProfile(BaseModel):
    ...
    registration_number = models.CharField(
        _("company registration"), null=True
    )
...
class UserAccount(AbstractBaseUser, PermissionsMixin, BaseModel):
    ...
    first_name = models.CharField(
        _("first name"), max_length=254, help_text="First Name of the User"
    )
    last_name = models.CharField(
        _("last name"), max_length=254, help_text="Last Name of the User"
    )
    ...
    role = models.ForeignKey(
        Role, on_delete=models.CASCADE, null=True, related_name="role_users"
    )
    company = models.ForeignKey(
        CompanyProfile,
        on_delete=models.CASCADE,
...
class Invitation(BaseModel):
    ...
    inviter_id = models.ForeignKey(
        UserAccount, on_delete=models.CASCADE, related_name="invitations"
    )
    first_name = models.CharField(_("first name"), max_length=254)
    last_name = models.CharField(_("last name"), max_length=254)
    email = models.EmailField(_("email"), max_length=255, unique=True)
    role = models.ForeignKey(
        Role, on_delete=models.CASCADE, related_name="invitations"
    )
    tenant = models.ForeignKey(
        "tenant.Tenant", on_delete=models.CASCADE, related_name="invitations"
    )
